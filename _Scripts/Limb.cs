...
public class Limb : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (_myAgent.closeFood != null)
        {
            ...
            {
                case ForceType.R_F:
                    rb.AddForce(Vector2.up * power * Mathf.Cos(angle) + dir.normalized * power / 5, ForceMode2D.Force);
                    break;
                case ForceType.L_F:
                    rb.AddForce(Vector2.right * power * Mathf.Cos(angle) + dir.normalized * power / 5, ForceMode2D.Force);
                    break;
                case ForceType.F_F:
                    rb.AddForce(Vector2.left * power * Mathf.Cos(angle) + dir.normalized * power / 5, ForceMode2D.Force);
                    break;
                case ForceType.R_R:
                    rb.AddForce(Vector2.down * power * Mathf.Cos(angle) + dir.normalized * power / 5, ForceMode2D.Force);
                    break;
                case ForceType.L_R:
                    rb.AddForce(Vector2.up * power * Mathf.Cos(angle) + dir.normalized * power / 5, ForceMode2D.Force);
                    break;
